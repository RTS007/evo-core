...
- Q: How should Real-Time thread properties be configured? → A: **External Management** - Out of scope for HAL Sim. An external supervisor (e.g., `evo-watchdog`) or systemd unit handles `chrt`/`task set`. HAL Sim just runs.
